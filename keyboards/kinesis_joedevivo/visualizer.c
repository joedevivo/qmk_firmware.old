...
static uint8_t get_led_index(visualizer_user_data_t* user_data) {
    for (int i=0; i < 3; i++) {
...
static void update_emulated_leds(visualizer_state_t* state, visualizer_keyboard_status_t* prev_status) {
    ...
    uint8_t new_index = get_led_index(user_data_new);
    uint8_t old_index = get_led_index(user_data_old);
